...
import { updateDraftRules } from '../rules/update-draft.rules';
import { UpdateDraftUseCase } from '../use-cases/update-draft.use-case';

export const updateDraftController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const schemaValidationResponse = SchemaValidationService.validate(controllerInput.req['body'], updateDraftRules);
    ...
    const updateDraftUseCase = new UpdateDraftUseCase({
      updateRepository,
      retrieveRepository,
    ...
    const response = await updateDraftUseCase.handle({
