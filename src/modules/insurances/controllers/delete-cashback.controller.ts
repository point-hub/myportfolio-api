...
import { DeleteCashbackRepository } from '../repositories/delete-cashback.repository';
import { RetrieveRepository } from '../repositories/retrieve.repository';
import { DeleteCashbackUseCase } from '../use-cases/delete-cashback.use-case';

export const deleteCashbackController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const deleteCashbackRepository = new DeleteCashbackRepository(controllerInput.dbConnection, { session });
    ...
    const response = await deleteCashbackUseCase.handle({
