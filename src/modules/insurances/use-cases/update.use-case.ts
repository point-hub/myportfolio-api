...
export class UpdateUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    // Check whether the user is authorized to perform this action
    const isAuthorized = this.deps.authorizationService.hasAccess(input.authUser.role?.permissions, 'insurances:update');
    if (!isAuthorized) {
    ...
    if (!insuranceEntity.data.interest?.is_rollover && roundNumber(totalInterestAmount ?? 0, 4) !== insuranceEntity.data.interest?.net_amount) {
      return this.fail({ code: 400, message: `Total interest schedule amount (${roundNumber(totalInterestAmount ?? 0, 4)}) does not match net amount (${insuranceEntity.data.interest?.net_amount}).` });
    }

    // Reject update when no fields have changed
    const changes = this.deps.auditLogService.buildChanges(
      retrieveResponse,
      this.deps.auditLogService.mergeDefined(retrieveResponse, insuranceEntity.data),
    );
    ...
    // Save the data to the database.
    const response = await this.deps.updateRepository.handle(input.filter._id, insuranceEntity.data);
