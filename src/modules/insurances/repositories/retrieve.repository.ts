...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IInsurance | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IInsurance | null> {
    const response = await this.database.collection(collectionName).retrieve<IInsurance>(_id, this.options);
    if (!response) {
      return null;
    }
