...
export class UpdateUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    ...
    if (!depositEntity.data.interest?.is_rollover && roundNumber(totalInterestAmount ?? 0, 4) !== depositEntity.data.interest?.net_amount) {
      return this.fail({ code: 400, message: `Total interest schedule amount (${roundNumber(totalInterestAmount ?? 0, 4)}) does not match net amount (${depositEntity.data.interest?.net_amount}).` });
    }

    // Reject update when no fields have changed
    const changes = this.deps.auditLogService.buildChanges(
      retrieveResponse,
      this.deps.auditLogService.mergeDefined(retrieveResponse, depositEntity.data),
    );
    ...
    // Save the data to the database.
    const response = await this.deps.updateRepository.handle(input.filter._id, depositEntity.data);
