...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IDeposit | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IDeposit | null> {
    const response = await this.database.collection(collectionName).retrieve<IDeposit>(_id, this.options);
    if (!response) {
      return null;
    }
