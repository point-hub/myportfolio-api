...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IBroker | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IBroker | null> {
    const response = await this.database.collection(collectionName).retrieve<IBroker>(_id, this.options);
    if (!response) {
      return null;
    }
