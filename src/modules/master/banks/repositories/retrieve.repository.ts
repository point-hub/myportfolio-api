...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IBank | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IBank | null> {
    const response = await this.database.collection(collectionName).retrieve<IBank>(_id, this.options);
    if (!response) {
      return null;
    }
