...
import type { IRetrieveAccountsRepository } from '../repositories/retrieve-accounts.repository';
...
export interface IDeps {
  retrieveAccountsRepository: IRetrieveAccountsRepository
 ...
 */
export class RetrieveAccountsUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    // Check whether the user is authorized to perform this action
    const isAuthorized = this.deps.authorizationService.hasAccess(input.authUser.role?.permissions, 'banks:read');
    if (!isAuthorized) {
      return this.fail({ code: 403, message: 'You do not have permission to perform this action.' });
    }

    // Retrieve all data from the database.
    const response = await this.deps.retrieveAccountsRepository.handle(input.query);
