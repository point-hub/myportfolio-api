...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IOwner | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IOwner | null> {
    const response = await this.database.collection(collectionName).retrieve<IOwner>(_id, this.options);
    if (!response) {
      return null;
    }
