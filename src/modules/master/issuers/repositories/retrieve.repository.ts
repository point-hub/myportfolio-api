...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IIssuer | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IIssuer | null> {
    const response = await this.database.collection(collectionName).retrieve<IIssuer>(_id, this.options);
    if (!response) {
      return null;
    }
