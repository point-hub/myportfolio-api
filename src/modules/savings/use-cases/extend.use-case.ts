...
export class ExtendUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    // Check whether the user is authorized to perform this action
    const isAuthorized = this.deps.authorizationService.hasAccess(input.authUser.role?.permissions, 'savings:renew');
    if (!isAuthorized) {
    ...
    // Check the insterest schedule amount should match with interest net amount.
    const totalInterestAmount = savingEntity.data.interest_schedule?.reduce(
      (sum, item) => sum + (item.amount || 0),
      0,
    );
    if (!savingEntity.data.interest?.is_rollover && roundNumber(totalInterestAmount ?? 0, 3) !== savingEntity.data.interest?.net_amount) {
      return this.fail({ code: 400, message: `Total interest schedule amount (${roundNumber(totalInterestAmount ?? 0, 3)}) does not match net amount (${savingEntity.data.interest?.net_amount}).` });
    }
