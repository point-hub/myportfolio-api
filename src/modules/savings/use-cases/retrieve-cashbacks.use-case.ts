...
import type { IRetrieveCashbacksRepository } from '../repositories/retrieve-cashbacks.repository';
...
export interface IDeps {
  retrieveCashbacksRepository: IRetrieveCashbacksRepository
...
/**
 * Use case: Retrieve Saving Cashbacks.
 *
 * Responsibilities:
 ...
 */
export class RetrieveCashbacksUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    // Check whether the user is authorized to perform this action
    const isAuthorized = this.deps.authorizationService.hasAccess(input.authUser.role?.permissions, 'savings:read');
    if (!isAuthorized) {
      return this.fail({ code: 403, message: 'You do not have permission to perform this action.' });
    }

    // Retrieve all data from the database.
    const response = await this.deps.retrieveCashbacksRepository.handle(input.query);
