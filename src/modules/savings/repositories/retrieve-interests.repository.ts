...
export interface IRetrieveInterestsRepository {
...
export class RetrieveInterestsRepository implements IRetrieveInterestsRepository {
  constructor(
  ...
  async handle(query: IQuery): Promise<IRetrieveManyOutput> {
    ...
    pipeline.push(...this.pipeJoinBankAccount(
      'interest_schedule.bank_id',
      'interest_schedule.bank_account_uuid',
      'interest_schedule.bank',
    ));
    pipeline.push(...this.pipeJoinBankAccount(
      'interest_schedule.additional_bank_id',
      'interest_schedule.additional_bank_account_uuid',
      'interest_schedule.additional_bank',
    ));
  ...
  async raw(query: IQuery): Promise<IRetrieveManyRawOutput> {
    return await this.database.collection(collectionName).retrieveMany<ISaving>(query, this.options);
  }
  ...
  private pipeFlatInterestSchedule(): IPipeline[] {
    return [
      {
        $unwind: {
          path: '$interest_schedule',
          preserveNullAndEmptyArrays: false, // drop savings without schedule
        },
      },
    ];
  }
  ...
  private pipeQueryFilter(query: IQuery): IPipeline[] {
    ...
    if (query?.['search.all']) {
      ...
      const fields = ['form_number', 'owner.name', 'group.name', 'placement.account_number'];
      filters.push({
    ...
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'placement.account_number', query?.['search.placement.account_number']);
    ...
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.bank.name', query?.['search.interest_schedule.bank.name']);
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.bank.account.account_number', query?.['search.interest_schedule.bank.account.account_number']);
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.bank.account.account_name', query?.['search.interest_schedule.bank.account.account_name']);
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.additional_bank.name', query?.['search.interest_schedule.additional_bank.name']);
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.additional_bank.account.account_number', query?.['search.interest_schedule.additional_bank.account.account_number']);
    BaseMongoDBQueryFilters.addRegexFilter(filters, 'interest_schedule.additional_bank.account.account_name', query?.['search.interest_schedule.additional_bank.account.account_name']);
    ...
    addDateRangeFilter(filters, 'interest_schedule.payment_date', query?.['search.interest_schedule.payment_date_from'], query?.['search.interest_schedule.payment_date_to']);
    addDateRangeFilter(filters, 'interest_schedule.received_date', query?.['search.interest_schedule.received_date_from'], query?.['search.interest_schedule.received_date_to']);
    addDateRangeFilter(filters, 'interest_schedule.received_additional_payment_date', query?.['search.interest_schedule.received_additional_payment_date_from'], query?.['search.interest_schedule.received_additional_payment_date_to']);
    ...
    BaseMongoDBQueryFilters.addNumberFilter(filters, 'interest_schedule.amount', query?.['search.interest_schedule.amount']);
    BaseMongoDBQueryFilters.addNumberFilter(filters, 'interest_schedule.received_amount', query?.['search.interest_schedule.received_amount']);
    BaseMongoDBQueryFilters.addNumberFilter(filters, 'interest_schedule.received_additional_payment_amount', query?.['search.interest_schedule.received_additional_payment_amount']);
    ...
    } else if (query?.['search.interest_schedule.status'] && query?.['search.interest_schedule.status'] === 'false') {
      filters.push({ 'interest_schedule.received_date': { $exists: false } });
    }
    ...
    return [
      {
        $lookup: {
          from: 'users',
          let: {
            userId: '$interest_schedule.created_by_id',
          },
          pipeline: [
            {
              $match: {
          ...
          as: 'interest_schedule.created_by',
        },
      },
      {
        $unwind: {
          path: '$interest_schedule.created_by',
          preserveNullAndEmptyArrays: true,
        },
      },
    ];
