...
import { withdrawalRules } from '../rules/withdrawal.rules';
import { WithdrawalUseCase } from '../use-cases/withdrawal.use-case';

export const withdrawalController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const schemaValidationResponse = SchemaValidationService.validate(controllerInput.req['body'], withdrawalRules);
    ...
    const withdrawalUseCase = new WithdrawalUseCase({
      updateRepository,
      retrieveRepository,
    ...
    const response = await withdrawalUseCase.handle({
