...
import { ReceiveInterestRepository } from '../repositories/receive-interest.repository';
import { RetrieveRepository } from '../repositories/retrieve.repository';
import { receiveInterestRules } from '../rules/receive-interest.rules';
import { ReceiveInterestUseCase } from '../use-cases/receive-interest.use-case';

export const receiveInterestController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const schemaValidationResponse = SchemaValidationService.validate(controllerInput.req['body'], receiveInterestRules);
    ...
    const receiveInterestRepository = new ReceiveInterestRepository(controllerInput.dbConnection, { session });
    ...
    const receiveInterestUseCase = new ReceiveInterestUseCase({
      receiveInterestRepository,
      retrieveRepository,
    ...
    const response = await receiveInterestUseCase.handle({
