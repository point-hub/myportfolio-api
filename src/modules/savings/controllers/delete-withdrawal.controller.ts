...
import { DeleteWithdrawalUseCase } from '../use-cases/delete-withdrawal.use-case';

export const deleteWithdrawalController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const deleteWithdrawalUseCase = new DeleteWithdrawalUseCase({
      updateRepository,
      retrieveRepository,
    ...
    const response = await deleteWithdrawalUseCase.handle({
